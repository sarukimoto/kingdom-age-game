...
local function updateDetails(itemId, descriptions, purchaseStats, saleStats)
  ...
  if table.empty(saleStats) then
    sellStatsTable:addRow({{text = tr('No information')}})
  else
    local offerAmount = 0
    ...
    sellStatsTable:addRow({{text = tr('Total Transations') .. ':'}, {text = transactions}})
    sellStatsTable:addRow({{text = tr('Highest Price') .. ':'}, {text = highestPrice}})

    if totalPrice > 0 and transactions > 0 then
      sellStatsTable:addRow({{text = tr('Average Price') .. ':'},
        {text = math.floor(totalPrice/transactions)}})
    else
      sellStatsTable:addRow({{text = tr('Average Price') .. ':'}, {text = 0}})
    end

    sellStatsTable:addRow({{text = tr('Lowest Price') .. ':'}, {text = lowestPrice}})
  end

  -- update buy item statistics
  buyStatsTable:clearData()
  if table.empty(purchaseStats) then
    buyStatsTable:addRow({{text = tr('No information')}})
  else
    local transactions, totalPrice, highestPrice, lowestPrice = 0, 0, 0, 0
    ...
    buyStatsTable:addRow({{text = tr('Total Transations') .. ':'},{text = transactions}})
    buyStatsTable:addRow({{text = tr('Highest Price') .. ':'}, {text = highestPrice}})

    if totalPrice > 0 and transactions > 0 then
      buyStatsTable:addRow({{text = tr('Average Price') .. ':'},
        {text = math.floor(totalPrice/transactions)}})
    else
      buyStatsTable:addRow({{text = tr('Average Price') .. ':'}, {text = 0}})
    end

    buyStatsTable:addRow({{text = tr('Lowest Price') .. ':'}, {text = lowestPrice}})
  end
end
...
local function initMarketItems()
  ...
  for i = 1, #types do
    ...
    if item then
      ...
      if not table.empty(marketData) and not itemSet[marketData.tradeAs] and marketItems[marketData.category] then
...
function Market.clearSelectedItem()
  if Market.isItemSelected() then
    ...
    offerTypeList:clearOptions()
    offerTypeList:setText(tr('Select'))
...
function Market.createNewOffer()
  ...
  if type == MarketAction.Buy then
    if information.balance < ((piecePrice * amount) + fee) then
      errorMsg = errorMsg .. tr('Not enough balance to create this offer.') .. '\n'
    end
  elseif type == MarketAction.Sell then
    ...
    if Market.getDepotCount(spriteId) < amount then
      errorMsg = errorMsg .. tr('Not enough items in your depot to create this offer.') .. '\n'
    end
  end

  if piecePrice > piecePriceEdit.maximum then
    errorMsg = errorMsg .. tr('Price is too high.') .. '\n'
  elseif piecePrice < piecePriceEdit.minimum then
    errorMsg = errorMsg .. tr('Price is too low.') .. '\n'
  end

  if amount > amountEdit.maximum then
    errorMsg = errorMsg .. tr('Amount is too high.') .. '\n'
  elseif amount < amountEdit.minimum then
    errorMsg = errorMsg .. tr('Amount is too low.') .. '\n'
  end

  if amount * piecePrice > MarketMaxPrice then
    errorMsg = errorMsg .. tr('Total price is too high.') .. '\n'
  end

  if information.totalOffers >= MarketMaxOffers then
    errorMsg = errorMsg .. tr('You cannot create more offers.') .. '\n'
  end
  ...
  if timeCheck < offerExhaust[type] then
    ...
    errorMsg = errorMsg .. tr('You must wait %d seconds before creating a new offer.', waitTime) .. '\n'
  end
