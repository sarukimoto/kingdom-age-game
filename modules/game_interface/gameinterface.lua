...
function init()
  ...
  hideChatButton = gameRootPanel:getChildById('hideChatButton')
  hideTopMenuButton = gameRootPanel:getChildById('hideTopMenuButton')
...
function bindKeys()
  ...
  g_keyboard.bindKeyPress('Ctrl+Up', function() if not g_game.canHoldDirectionChange(North) then return end g_game.turn(North) changeWalkDir(North) end, gameRootPanel)
  g_keyboard.bindKeyPress('Ctrl+Right', function() if not g_game.canHoldDirectionChange(East) then return end g_game.turn(East) changeWalkDir(East) end, gameRootPanel)
  g_keyboard.bindKeyPress('Ctrl+Down', function() if not g_game.canHoldDirectionChange(South) then return end g_game.turn(South) changeWalkDir(South) end, gameRootPanel)
  g_keyboard.bindKeyPress('Ctrl+Left', function() if not g_game.canHoldDirectionChange(West) then return end g_game.turn(West) changeWalkDir(West) end, gameRootPanel)
  g_keyboard.bindKeyPress('Ctrl+Numpad8', function() if not g_game.canHoldDirectionChange(North) then return end g_game.turn(North) changeWalkDir(North) end, gameRootPanel)
  g_keyboard.bindKeyPress('Ctrl+Numpad6', function() if not g_game.canHoldDirectionChange(East) then return end g_game.turn(East) changeWalkDir(East) end, gameRootPanel)
  g_keyboard.bindKeyPress('Ctrl+Numpad2', function() if not g_game.canHoldDirectionChange(South) then return end g_game.turn(South) changeWalkDir(South) end, gameRootPanel)
  g_keyboard.bindKeyPress('Ctrl+Numpad4', function() if not g_game.canHoldDirectionChange(West) then return end g_game.turn(West) changeWalkDir(West) end, gameRootPanel)
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature)
  ...
  local isMouseLeftRightPressed = g_mouse.isPressed(MouseLeftButton) and mouseButton == MouseRightButton or g_mouse.isPressed(MouseRightButton) and mouseButton == MouseLeftButton
  ...
  if not modules.client_options.getOption('classicControl') then
    if keyboardModifiers == KeyboardNoModifier and mouseButton == MouseRightButton and not g_mouse.isPressed(MouseLeftButton) then
      ...
      return true
    elseif creatureThing and getDistanceBetween(creatureThing:getPosition(), player:getPosition()) >= 1 and (creatureThing:getPosition().z == autoWalkPos.z and g_keyboard.isCtrlPressed() and g_keyboard.isShiftPressed() and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) or not creatureThing:isMonster() and isMouseLeftRightPressed) then
      g_game.follow(creatureThing)
      return true
    elseif attackCreature and getDistanceBetween(attackCreature:getPosition(), player:getPosition()) >= 1 and (g_keyboard.isAltPressed() and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) or attackCreature:isMonster() and isMouseLeftRightPressed) then
      g_game.attack(attackCreature)
      return true
    elseif creatureThing and getDistanceBetween(creatureThing:getPosition(), player:getPosition()) >= 1 and (creatureThing:getPosition().z == autoWalkPos.z and g_keyboard.isAltPressed() and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) or creatureThing:isMonster() and isMouseLeftRightPressed) then
      g_game.attack(creatureThing)
      return true
    elseif useThing and (keyboardModifiers == KeyboardCtrlModifier and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) or isMouseLeftRightPressed) then
      if useThing:isContainer() then
        if useThing:getParentContainer() and not isMouseLeftRightPressed then
...
function getHideTopMenuButton()
  return hideTopMenuButton
end
...
function setupViewMode(mode)
  ...
  if currentViewMode == 2 then
    ...
    hideTopMenuButton:setMarginTop(5)
  ...
  elseif mode == 2 then
    ...
    hideTopMenuButton:setMarginTop((isTopMenuVisible and topMenu:getHeight() - gameRightPanel:getPaddingTop() or 0) + 5)
